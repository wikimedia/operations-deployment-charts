...
## Running tests
Tests can be run via Make in the service directory.
The `env` parameter selects which environment to test against:
...
When invoked from the Makefile in the api-gateway chart's directory
(`charts/api-gateway/`), `env=minikube` is applied automatically.
...
You can also use `make env` to get the necessary environment.
