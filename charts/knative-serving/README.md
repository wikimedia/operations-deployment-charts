...
knative-serving: 1.21.1 (VERSION)
knative-net-istio: 1.21.1 (ISTIO-VERSION)
...
By default, Knative Serving requires a recent Kubernetes version. To force
compatibility with Kubernetes 1.31, the `KUBERNETES_MIN_VERSION` environment
variable is injected into the controller and webhook deployments via the
`.Values.kubernetes_min_version` value.
...
We keep the webhook probes more conservative than upstream to avoid premature
restarts during rollout in our clusters. The problem seems to be
https://github.com/knative/serving/pull/9661, but the upstream values are not
enough for our use case.

Starting from the 1.21 line, upstream no longer ships standalone domain-mapping
controller and webhook deployments in serving-core. The corresponding chart
resources and values can be removed when refreshing from upstream.
