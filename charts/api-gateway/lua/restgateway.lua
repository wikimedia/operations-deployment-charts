...
function wmf_stash_headers(request_handle)
    ...
    -- Make additional request information available to response hooks.
    -- Needed by wmf_set_cors_access_control.
    local extra_headers = { ":path", ":method", "origin" }
    for _, hname in ipairs(extra_headers) do
        local hvalue = req_headers:get(hname)
...
function wmf_expose_headers(response_handle)
    ...
    local req_headers = streamMeta:get("envoy.wmf_headers")
    local exposed_headers = HelmValues.main_app.ratelimiter.exposed_headers
...
function wmf_set_cors_access_control(response_handle)
    ...
    local method = req_headers[":method"]
